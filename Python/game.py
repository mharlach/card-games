...
class PokerGame:
    ...
    def showdown(self):
        ...
        for p in self.players:
            hand = evaluatePokerHand(p.cards)
...
class PokerHand:
    ...
    def toString(self):
        text = "{a}.{b}.{c}".format(a=self.handValue, b=self.handCardValue, c=self.kicker)
        return text
...
def evaluatePokerHand(hand):
    ...
    if multiples:
        ...
        if len(pairs) == 1:
            kicker = max([c.cardValue for c in cards if c.cardValue != pairs[0]])
